...
# Add parent directory to path to locate dependencies
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

# Dynamically import the sales assistant graph
sales_module = importlib.import_module("deepAgentCourse.5_sales_assistant")
graph = sales_module.graph

# Session tracking state
session_thread = {"configurable": {"thread_id": "gradio-sales-thread"}}
...
def handle_customer_query(query, chat_history):
    ...
    print(f"--- Gradio Invoking Graph for Query: '{query}' ---")
...
def approve_and_resume(chat_history):
    ...
    print("--- Gradio: Manager Approved. Resuming Graph ---")
